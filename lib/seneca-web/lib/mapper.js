  ...
  _.each(routePlan, routeSet => {
    _.mapKeys(routeSet.map, (value, key) => {
      ...
      var defaultRoute = {
        ...
        middleware: false,
        secure: false,
        request$: value.request$ || false,  // Mark
        response$: value.response$ || false  // Mark
      }

      if (routeSet.middleware) {
        defaultRoute.middleware = _.isArray(routeSet.middleware)
          ? routeSet.middleware
          : [routeSet.middleware]
      }
      ...
      // Set alias', suffix, middleware and redirects.
      route.alias = value.alias || route.alias
      route.redirect = value.redirect || route.redirect
      route.suffix = value.suffix || route.suffix

      if (value.middleware) {
        route.middleware = _.concat(
          route.middleware || [],
          _.isArray(value.middleware) ? value.middleware : [value.middleware]
        )
      }
